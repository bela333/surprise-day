...
async def main():
    ...
    async def reset_surpriseday():
        for day in await SurpriseDay.get_expired(database, datetime.now()):
            if day.channel is None:
                continue

            surprise_day, reset_day = SurpriseDay.generate_surpriseday_and_resetday()
            await day.update_surprise_day(database, surprise_day)
            await day.update_reset_day(database, reset_day)
            ...
            if day.message is not None:
                try:
                    ...
                    pass
            await day.update_message(database, str(msg.id))
    ...
    async def handle_join(member: hikari.Member, guild: hikari.SnowflakeishOr[hikari.PartialGuild]) -> None:
        ...
        day = await SurpriseDay.get_from_discord_or_default(database, str(member.id))
        ...
        await day.update_channel(database, str(channel.id))
    ...
    async def handle_leave(user_id: hikari.Snowflake, guild: hikari.SnowflakeishOr[hikari.PartialGuild]) -> None:
        guild = hikari.Snowflake(guild)
        day = await SurpriseDay.get_from_discord(database, str(user_id))
        if day is None or day.channel is None:
            return
        await bot.rest.delete_channel(hikari.Snowflake(day.channel))
        await day.update_channel(database, None)
        await day.update_message(database, None)
    ...
    async def on_start(event: hikari.StartingEvent) -> None:
        # Setup database
        await SurpriseDay.setup(database)
        # Start `reset` job
